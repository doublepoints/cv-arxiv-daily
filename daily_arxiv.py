...
def get_daily_papers(topic,query="slam", max_results=2):
    ...
    for result in client.results(search_engine):
        ...
        # Best-effort code-link lookup. The arxiv.paperswithcode.com endpoint
        # is currently unreachable (TLS handshake failure); on any error we
        # fall back to "null" so the paper still gets recorded.
        repo_url = None
        try:
            ...
            logging.warning(f"paperswithcode lookup failed for {paper_key}: {e}")
...
def update_json_file(filename,data_dict,config_keywords):
    '''
    ...
    json_data = {k: v for k, v in m.items() if k in config_keywords}
...
def json_to_md(filename,md_filename,
    ...
    with open(md_filename,"a+") as f:
        top_anchor = "top"
        if use_title == True:
            f.write(f"## <a id={top_anchor}></a>Updated on {DateNow}\n")
        else:
            f.write(f"> <a id={top_anchor}></a>Updated on {DateNow}\n")
        ...
        if use_tc == True:
            ...
            for keyword in data.keys():
                ...
                if not day_content:
                    continue
                ##kw = keyword.replace(' ','-')
                anchor = keyword.lower().replace(' ', '-').replace('.', '')
                ##f.write(f"    <li><a href=#{kw}>{keyword}</a></li>\n")
                f.write(f"    <li><a href=#{anchor}>{keyword}</a></li>\n")
            f.write("  </ol>\n")
            f.write("</details>\n\n")
        ...
        for keyword in data.keys():
            ...
            ##f.write(f"## {keyword}\n\
            anchor = keyword.lower().replace(' ', '-').replace('.', '')
            f.write(f"## <a id={anchor}>{keyword}</a>\n\n")
